<!doctype html>
<html>
<head>
    <title>Food App</title>
    <link rel="stylesheet" href="${resource(dir: 'css', file: 'text.css')}" type = "text/css">
    <link rel="stylesheet" href="${resource(dir: 'css', file: 'jquery-jvectormap-2.0.3.css')}" type="text/css" media="screen"/>
    <script src="../../src/main/js/jquery-jvectormap-2.0.3.min.js"></script>
    <script src="../../src/main/js/jquery-jvectormap-world-mill.js"></script>
    <script src="../../src/main/js/jquery-3.1.1.min.js"></script>

</head>
<body>
<div id="root" align="left"></div>
<br/>
<asset:javascript src="bundle.js"/>
</body>
</html>